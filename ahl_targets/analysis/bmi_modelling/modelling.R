...
df_3 <- df %>%
  ...
  mutate(intake_diff = -216) #create new column for kcal reduction
...
bmi_class_dist %>%
  melt() %T>%
  write_csv(here("outputs", "reports", "chart_csv", "chartH.csv") %>% str_remove("ahl_targets/analysis/bmi_modelling/"))
