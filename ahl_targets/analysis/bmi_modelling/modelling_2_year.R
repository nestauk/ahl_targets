...
eichange_3 <- t(apply(df_3, 1, function(x) rep(as.numeric(x["intake_diff"]), 365*2))) #apply energy intake change over 3 years

model_weight_3 <- adult_weight(df_3$weight, df_3$height/100, df_3$age, df_3$sex, eichange_3, days=365*2) #calculate final body weight

bw_3 <- model_weight_3$Body_Weight %>% as.data.frame() %>% select(V730) %>% rename(final_weight = V730) #convert final body weight list into dataframe
...
eichange_4 <- t(apply(df_4, 1, function(x) rep(as.numeric(x["intake_diff"]), 365*2)))

model_weight_4 <- adult_weight(df_4$weight, df_4$height/100, df_4$age, df_4$sex, eichange_4, days=365*2)

bw_4 <- model_weight_4$Body_Weight %>% as.data.frame() %>% select(V730) %>% rename(final_weight = V730)
...
eichange_5 <- t(apply(df_5, 1, function(x) rep(as.numeric(x["intake_diff"]), 365*2)))

model_weight_5 <- adult_weight(df_5$weight, df_5$height/100, df_5$age, df_5$sex, eichange_5, days=365*2)
bw_5 <- model_weight_5$Body_Weight %>% as.data.frame() %>% select(V730) %>% rename(final_weight = V730)
...
bmi_class_dist %>%
  melt() %T>%
  write_csv(here("outputs", "reports", "chart_csv", "two_year.csv") %>% str_remove("ahl_targets/analysis/bmi_modelling/"))
